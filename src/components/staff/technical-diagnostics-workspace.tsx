...
function cards(data: TechnicalDiagnostics): Card[] {
  return [
    ...
    { title: "Cloudflare deployment", status: data.deployment.status, metrics: { latest_commit: data.deployment.commit || "—", deployed: data.deployment.deployed_at ? new Date(data.deployment.deployed_at).toLocaleString("en-IN") : "—" }, description: "Reports the latest production Pages deployment when provider access is configured." },
