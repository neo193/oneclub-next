...
export function ResetPasswordForm() {
  ...
  return <form className="contact-form auth-form" onSubmit={submit}><div className="form-heading"><span>New credentials</span><h3>Update your password.</h3></div><label>New password<PasswordInput name="password" autoComplete="new-password" required minLength={8}/></label><label>Confirm new password<PasswordInput name="confirm_password" autoComplete="new-password" required minLength={8}/></label><button className="button button-primary" type="submit" disabled={pending}>{pending?"Updating…":"Update password"}</button><p className="form-message" aria-live="polite">{message}</p></form>;
}
