...
async function requireMembershipPaymentState(userId:string){const profiles=await rest(`profiles?select=membership_state,membership_plan,payment_offer_expires_at&id=eq.${userId}`);const profile=profiles[0];if(profile?.membership_state==="active"&&profile.membership_plan==="annual")return profile;if(profile?.membership_state!=="payment_pending")throw new Error("Membership purchase is unavailable");if(new Date(profile.payment_offer_expires_at||0)<=new Date())throw new Error("This membership payment offer has expired");return profile;}
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(input.action==="create"){
      let amount:number,bookingId:null|string=null,description:string,membershipPlan:null|string=null,membershipCredit=0;
      if(input.purpose==="membership"){
        const prepared=await rest("rpc/prepare_membership_payment",{method:"POST",body:JSON.stringify({p_member_id:user.id,p_plan:input.membership_plan})});amount=prepared.amount_paise;membershipPlan=prepared.plan;membershipCredit=prepared.credit_paise||0;description=membershipPlan==="annual"?"One Club Annual Membership":"One Club Founding Membership";
      ...
      const attempts=await rest("payment_attempts",{method:"POST",headers:{Prefer:"return=representation"},body:JSON.stringify({user_id:user.id,purpose:input.purpose,booking_id:bookingId,amount_paise:amount,razorpay_order_id:order.id,membership_plan:membershipPlan,membership_credit_paise:membershipCredit})});
      return json({attempt_id:attempts[0].id,key_id:razorpayKey,order_id:order.id,amount,currency:"INR",description});
    }
    if(input.action==="reconcile"){
      if(input.purpose==="membership")await requireMembershipPaymentState(user.id);
    ...
    if(input.action==="verify"){
      ...
      if(attempt.purpose==="membership")await requireMembershipPaymentState(user.id);
