...
create index if not exists member_admin_notes_member_created_idx
  on public.member_admin_notes(member_id, created_at desc);

alter table public.member_admin_notes enable row level security;
revoke all on table public.member_admin_notes from public, anon, authenticated;
...
create or replace function public.add_member_admin_note(p_member_id uuid,p_note text)
returns uuid
language plpgsql security definer set search_path=public,pg_temp
as $$
declare v_id uuid;
begin
  if not public.can_manage_members() then raise exception 'Member management permission required'; end if;
  if char_length(trim(coalesce(p_note,''))) not between 3 and 1000 then raise exception 'Note must be between 3 and 1000 characters'; end if;
  ...
  insert into public.member_admin_notes(member_id,author_id,note)
  values(p_member_id,auth.uid(),trim(p_note)) returning id into v_id;
  insert into public.audit_log(actor_id,action,entity_type,entity_id,details)
  values(auth.uid(),'member.internal_note_added','member',p_member_id::text,jsonb_build_object('note_id',v_id));
  return v_id;
end;
$$;
...
revoke all on function public.add_member_admin_note(uuid,text) from public,anon;
